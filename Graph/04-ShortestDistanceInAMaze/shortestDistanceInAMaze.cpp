// Just a little variation of previous problem --> only instead of starting point as {0, 0}  it is now given in the problem.
...
int shortestPath(vector<vector<int>> &grid, pair<int, int> source, pair<int, int> destination) {
    ...
    int sr = source.first; // Row index of the source cell
    int sc = source.second; // Column index of the source cell

    int X = destination.first; // Row index of the destination cell
    int Y = destination.second; // Column index of the destination cell

    if (grid[sr][sc] == 0) // If starting cell (source) contains 0, it's not possible to reach the target.
        return -1;

    vector<vector<int>> vis(N, vector<int>(M, 0)); // To keep track of visited cells
    queue<pair<int, int>> q; // Queue for BFS
    q.push({sr, sc}); // Add the source cell to the queue
...
int main() {
    ...
    pair<int, int> source1 = make_pair(0, 0);
    pair<int, int> destination1 = make_pair(4, 3);

    int steps1 = shortestPath(grid1, source1, destination1);
    if (steps1 != -1) {
        cout << "Sample Input 1: Steps to reach the target cell: " << steps1 << endl;
    } else {
        cout << "Sample Input 1: Target cell is not reachable." << endl;
    }

    // Sample input 2
    pair<int, int> destination2 = make_pair(2, 4);
    int steps2 = shortestPath(grid1, source1, destination2);
    if (steps2 != -1) {
        cout << "Sample Input 2: Steps to reach the target cell: " << steps2 << endl;
    } else {
